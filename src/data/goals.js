...
// profile: { age, gender: 'male' | 'female', mealsPerDay }
export function getDailyGoals(profile) {
...
export function getMealGoals(dailyGoals, mealsPerDay) {
  const n = Math.max(1, Number(mealsPerDay) || 3);
  const mealGoals = {};
  Object.keys(dailyGoals).forEach((k) => {
